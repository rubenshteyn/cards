      ...
      <div class="modal-dialog modal-dialog-centered mw-550px" :style="{transform: this.transform}">
        ...
        <div class="modal-content">
                  ...
                  <div class="card rounded-3">
                    <div class="card-body" style="padding: 2rem 1rem;">
                      ...
                      <form class="form w-100" :style="{ display: styles.replenishment }">
                        ...
                        <div class="form-group fv-row mb-8 fs-6 fw-semibold">
                          <div v-if="outputResultReplenishmentCard()">There are enough funds on the balance to issue
                            cards
                          </div>
                          <div v-else style="color:red;">There are not enough funds on the balance</div>
                        </div>
          ...
          <div class="modal-footer flex-center">
            ...
            <button :style="{ display: styles.creation }" :disabled="this.isDisabled" @click="checkForm()" type="submit" id="kt_modal_new_target_submit"
                    class="btn btn-primary">
              <span class="indicator-label">Save changes</span>
              <span class="indicator-progress">Please wait...
									<span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
            </button>
            <button :style="{ display: styles.replenishment }" :disabled="this.isDisabledReplenishmentCard" @click="checkForm()" type="submit" id="kt_modal_new_target_submit"
                    class="btn btn-primary">
              <span class="indicator-label">Save changes</span>
              <span class="indicator-progress">Please wait...
									<span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
...
const initialState = () => {
  return {
    ...
    systemType: null,
    isDisabled: true,
    isDisabledReplenishmentCard: true,
    activeTab: 'creation',
    styles: {
...
export default defineComponent({
  ...
  watch: {
    ...
    "selectedCard": "selectTypesCard"
  },
  methods: {
    selectTypesCard() {
      if (this.selectedCard && this.selectedCard.card.cardNumber.toString().includes('486555')) {
        this.cardType = {
          name: "Universal",
          commission: 8,
          sumToCreate: 15,
    ...
    outputResultReplenishmentCard() {
      console.log(this.cardType, this.selectedUser, this.isDisabled)
      ...
      if ((this.cardType && this.formReplenishmentCard.amount && this.selectedBalance) &&
          this.selectedBalance.balance > (this.cardType.sumToCreate) + (this.formReplenishmentCard.amount)) {
        this.isDisabledReplenishmentCard = false
        return true
      }
      if (this.selectedBIN && this.selectedUser.name && this.cardType && this.formReplenishmentCard.amount && this.selectedBalance.balance) {
        console.log(this.selectedBIN)
        this.field = false
      }
      this.isDisabledReplenishmentCard = true
    },
    outputResult() {
    ...
    ...mapActions({
      createCardByTask: "admin/createCardByTask",
      cardReplenishmentByTask: "admin/cardReplenishmentByTask"
    }),
    checkForm: function () {
      if (this.activeTab === "creation") {
        ...
        this.createCardByTask(this.formCreationCard)
            .then(() => {
              Object.assign(this.$data, initialState());
              this.closeModal()
              this.$swal({
    ...
    getData() {
      if (this.loggedIn && this.users.length === 0) {
