              ...
              <div class="mh-500px scroll-y me-n7 pe-7">
                <div class="card-body p-0">
                  ...
                  <div class="form-group fv-row mb-8">
                    ...
                    <v-select disabled class="bg-transparent " label="name" :options="balanceList"
...
export default {
  ...
  methods: {
    ...mapActions({
      topUpCardBalanceTask: "users/createTaskCardReplenishment"
    }),
    outputResult() {
    ...
    openModal(card) {
      ...
      this.balance = {
        "code": "USDT",
        "balance": this.userinfo.USDTBalance,
        "name": "USDT - " + this.usersBalance(this.userinfo.USDTBalance) + " ₮"
      }
    },
    closeModal() {
    ...
    commissionOutput() {
      if (this.cardNumber.toString().includes('486555') && this.userinfo.universalCommission) {
        return this.result = this.form.amount - ((this.form.amount * this.userinfo.universalCommission) / 100)
      }
      if (this.userinfo.advertisingCommission) {
        return this.result = this.form.amount - ((this.form.amount * this.userinfo.advertisingCommission) / 100)
      }
    ...
    checkForm() {
      ...
      this.topUpCardBalanceTask(this.form)
          .then(() => {
    ...
    balanceList() {
      ...
      if (this.userinfo) {
        balances = [
          ...
          {
            "code": "BTC",
            "balance": this.userinfo.BTCBalance,
            "name": "BTC - " + this.usersBalance(this.userinfo.BTCBalance) + " ₿"
          },
          {
            "code": "EUR",
            "balance": this.userinfo.EURBalance,
            "name": "EUR - " + this.usersBalance(this.userinfo.EURBalance) + " €"
          },
          {
            "code": "USD",
            "balance": this.userinfo.USDBalance,
            "name": "USD - " + this.usersBalance(this.userinfo.USDBalance) + " $"
          },
        ]
      }
      return balances
