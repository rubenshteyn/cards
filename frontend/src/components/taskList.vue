  ...
  <div class="d-flex flex-column user-tasks flex-column-fluid container-fluid">
    <div class="toolbar mb-5 mb-lg-7" id="kt_toolbar">
      ...
      <div class="page-title d-flex flex-column me-3">
        <!--begin::Title-->
        <h1 class="d-flex text-dark fw-bold my-1 fs-3">{{$t('tasks.title')}}</h1>
        <!--end::Title-->
        <!--begin::Breadcrumb-->
        <ul class="breadcrumb breadcrumb-dot fw-semibold text-gray-600 fs-7 my-1">
          ...
          <!--end::Item-->
          <!--begin::Item-->
          <li class="breadcrumb-item text-gray-500">{{$t('tasks.title')}}</li>
          <!--end::Item-->
        </ul>
        <!--end::Breadcrumb-->
      </div>
      ...
      <div class="actions-group-buttons d-flex align-items-center py-2 py-md-1 position-relative">
        ...
        <div class="me-3">
          ...
          <div id="filter-modal" v-show="isHidden"
            ...
            <!--begin::Header-->
            <div class="px-7 py-5">
              <div class="fs-5 text-dark fw-bold">{{$t('tasks.filterTasks')}}</div>
            </div>
            <!--end::Header-->
            ...
            <div class="px-7 py-5 scroll-y mh-500px">
              ...
              <div class="mb-10">
                <div class="form-group fv-row">
                  <label class="form-label fw-semibold" for="status">{{$t('cards.card')}}:</label>
              ...
              <div class="mb-10">
                <div class="form-group fv-row">
                  <label class="form-label fw-semibold" for="status">{{$t('tasks.statusTask')}}:</label>
        ...
        <!--end::Wrapper-->
        <!--begin::Button-->
        <button @click='showModal("create")' style="display:none;" class="btn btn-dark fw-bold" data-bs-toggle="modal"
                data-bs-target="#kt_modal_create_app"
                id="kt_toolbar_primary_button">Create task
        </button>
        <CreateTaskModal ref="create"/>
        <!--end::Button-->
      </div>
      <!--end::Actions-->
    </div>
    <div class="task mb-5 mb-xl-8">
      ...
      <div class="task-header align-items-center task-header-task border-0 pt-5">
        <h3 class="task-title align-items-start flex-column">
          <span class="d-block task-label fw-bold fs-3 mb-1">{{$t('tasks.allTasks')}}</span>
          <span class="d-block text-muted mt-1 fw-semibold fs-7"> {{$t('tasks.amount')}}: {{ this.filteredTasks.length }} </span>
        </h3>
      </div>
      <!--end::Header-->
      <!--begin::Body-->
        ...
        <div class="table-responsive">
          ...
          <table v-if="getTasks()" class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
            ...
            <tr class="fw-bold text-muted bg-light">
              ...
              <th @click="filterTaskBalance('balance')" class="min-w-150px cursor-pointer">Balance before</th>
                ...
                <div class="d-flex align-items-center">
                  <div v-if="task.status === 1" class="me-2">
                    <Popper class="fw-semibold fs-7" arrow hover placement="top" content="Click to cansel task">
                      <button @click='cancelMyTask(task.ID)'
...
export default {
  ...
  methods: {
    ...mapActions({
      cancelTask: "users/cancelTask",
      repeatTask: "users/repeatTask"
    }),
    showModal() {
    ...
    cancelMyTask(taskId) {
      ...
      this.cancelTask(form)
          .then(() => {
            this.$swal({
              title: this.$t('tasks.successCancel'),
    ...
    repeatTask(taskId) {
      ...
      this.repeatTask(form)
          .then(() => {
            this.$swal({
              title: this.$t('tasks.successRepeat'),
              confirmButtonColor: "#50cd89",
              type: "alert message",
              icon: "success",
              confirmButtonText: "OK"
    ...
    filter() {
      this.filteredTasks = this.tasks.filter(task => {
        ...
        if ((!this.filterItems.status.name || task.status === this.filterItems.status.code)
            && (!this.filterItems.selectedCard.code || task.cardId === this.filterItems.selectedCard.code)
    ...
    getTasks() {
      if (this.loggedIn && !this.tasksLoaded && this.req) {
        this.$store.dispatch("users/getTasksMethod");
      }
      if (this.loggedIn && !this.cardsLoaded && this.req) {
        this.$store.dispatch("users/getTasksMethod");
      }
      this.req = false
      return true;
  ...
  computed: {
    ...mapGetters({
      ...
      tasksLoaded: "users/getTasksLoaded",
